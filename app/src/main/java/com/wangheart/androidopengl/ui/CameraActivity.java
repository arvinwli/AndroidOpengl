...
public class CameraActivity extends BaseActivity {
    ...
    protected class MyRender implements GLSurfaceView.Renderer {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            texture = TexturesES.loadTexture(R.mipmap.box);
            try {
                shader = new ShaderES30(IOUtils.toString(getThis().getAssets().open("camera/box.vert")),
                        IOUtils.toString(getThis().getAssets().open("camera/box.frag")));
            } catch (IOException e) {
                e.printStackTrace();
            }
            shader.use();
